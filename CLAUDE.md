...
| **Mujicians** — Balatro-style music-theory deckbuilder: cards are notes (ROYGBIV), "poker hands" are chords/scales, score = theory correctness, and every hand is played as audio. *v1 vertical slice built (deck→hand→score→sound, each gig fills a repeating Mario-Paint-style backing loop, 3 gigs, Muse draft, daily cap); economy still to come. Demoted slice-1 note-grid preserved in `mujicians-compose.html`* | `mujicians.html` | [`docs/mujicians.md`](docs/mujicians.md) |
