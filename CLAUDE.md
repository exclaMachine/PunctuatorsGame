...
| **Mujicians** — Balatro-style music-theory deckbuilder: cards are notes (ROYGBIV), "poker hands" are chords/scales, score = theory correctness, and every hand is played as audio. *v1 vertical slice built (deck→hand→score→sound, each gig fills a repeating Mario-Paint-style backing loop, 3 gigs, Muse draft, daily cap; **Save-a-Song** = name/keep a gig's loop with a theory report card, replay/export from a Home Setlist); economy still to come. Demoted slice-1 note-grid preserved in `mujicians-compose.html`. **Notelings** letter-creature + Bestiary collection/story layer is designed but tentative/not built. A **7-movement progression campaign** (pitch→rhythm→dynamics→melody→harmony→timbre→structure, Codex-gated, today's game preserved as **Free Play**, daily-capped) has **Phases 0–2 + Phase 3 Stage 1 built** (MOVEMENTS registry; Campaign/Free-Play mode select; per-movement select-cap, scoring-term, deck-instrument & threshold gating; `persist.progress`; **the whole M1→M7 arc is playable** — every scoring term wired, M2 **rhythm figures over a 4-beat sub-bar grid** (unified `scheduleVoices` onset timing, beat-swept loop grid, figure-aware groove scoring + "play each figure" gate), per-hand p/mf/f Dynamics, 3-card melodic sequences, harmony stacks, timbre blends, and real per-mechanic advancement gates); the remaining depth (Phase 3 later stages = draftable figures/syncopation/explicit rests, Phase 4 cross-gig Structure) is not yet built — see the doc.* | `mujicians.html` | [`docs/mujicians.md`](docs/mujicians.md) |
