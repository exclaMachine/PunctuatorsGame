...
- **`protectedArticles` and `spoonerism()` used to run in every other mode** (`utils/utils.js`), which
  would have let Art the Tickler and The Foon join the team alongside the ladder heroes. **All three
  ladder modes suppressed them** (dev's call, 2026-08-25 — the `ladderMode` flag in
  `addSpansAndIdsForWordPlay`, later `heroManagedWords`): the mode is one word changing along the
  hierarchy, and a third and fourth hero rewriting the same sentence is noise on top of that. **Moot since
  2026-08-30** — the two of them now appear only in their own `articlesSpoonerisms` mode and the opt-out
  flag is gone ([`punctuators.md`](punctuators.md)) — but the consequence stands: nothing else fixes the
  article, so `landOnRung` makes the `a dog` → `an animal` text-node edit itself (`fixArticleBefore`,
...
| `utils/utils.js` | **BUILT** — `case "ladder":` in `addSpansAndIdsForWordPlay`; `targetId` in `heroToTheRescue`. *(The `ladderMode`/`heroManagedWords` opt-out free play joined on 2026-08-25 was **removed 2026-08-30**: Art the Tickler and The Foon now run only in their own mode, so there is nothing to suppress — §1.)* |
...
| `projectileAnchor` | `{x: 303, y: 76}` | `{x: 170, y: 211}` | **Where the shot leaves the drawn weapon**, as an offset from the top-left of the image *frame*, measured off the art's own alpha — so it has to be re-measured whenever the file or `heroScale` changes. Added 2026-08-27, because the engine's default spawn is the **frame's top-left**, and both frames are bottom-anchored (`restingY`) with every spare pixel piled up above the figure's head: Keen's figure occupies only `(232, 382)–(674, 1045)` of her 800 × 1045 frame, so her arrow was launching **211 px above the crossbow**, out of empty sky. General's bolt-from-nowhere was horizontal instead of vertical — his frame top *is* his sword tip, but the default x is the midline while the blade he is holding is off to the right. Keen's numbers: bolt tip at source `(393, 470)` → `(177, 212)` drawn at 0.45, less half the arrow's 14 px drawn width. General's: sword tip at source `(704, 170)` → `(317, 77)`, less half the broadsword's 27 px |
| `projectileStartPositionX` | `180` | `180` | **Now inert for these two** — kept only because the constructor takes it positionally. The rule it used to carry still binds every *un-anchored* hero: the shoot handlers spawn at `position.x + width - projectileStartPositionX` while the `Projectile`'s own `onload` rewrites it to `position.x + projectileStartPositionX`, and those two agree at half the drawn width and nowhere else, so any other value makes the shot jump sideways the instant the image loads. An anchor sidesteps it by making both sites compute the *same* point — which is what frees General's sword to fly from his raised blade rather than from his midline |
...
hold the same file). The scale and spawn numbers each hero now carries are derived from the art, not
chosen — see the table in §8 before swapping either file.

**Projectile spawn points — FIXED 2026-08-27.** Play showed Keen's arrow starting *far* above the
crossbow, and the cause was engine-wide rather than ladder-specific: every projectile is born at
`player.position.y`, which is the top of the hero's **image frame**, not its weapon. That is harmless
for art that fills its frame and badly wrong for art that doesn't — Keen's figure fills 442 × 663 of an
800 × 1045 frame, and since `restingY()` bottom-anchors the frame, all 382 rows of slack sit above her
head (~172 px on screen), with the crossbow a further 40 below that. The fix is a per-hero
`projectileAnchor` and one `Hero.projectileSpawn()` that both shoot handlers and `Projectile`'s
`onload` now call, so a spawn point is computed in exactly one place. **Un-anchored heroes are
bit-for-bit unchanged** — `projectileAnchor` defaults to `null` and each call site keeps its own
historical arithmetic in that branch — so the other 21 heroes were not touched. General was anchored in
the same pass: his error was horizontal (his shot came off the midline while his sword is raised to the
right), which does move his aim point ~123 px right of his body — one number to dial back if it plays
worse than it looks.
