...
**"bundled recorded phoneme clips"** option below is built — **35 clips at `sounds/phonemes/<ipa>.mp3`**
(24 consonants + 11 monophthong vowels; the 5 diphthongs reuse their **starting-vowel** clip), hand-fetched
from **Wikimedia Commons** by **`fetch-phoneme-samples.sh`** (Commons names each file by articulatory
description; the script renames the mp3 to the IPA symbol so it matches `PHONEMES_BY_IPA`, then trims / mono /
loudness-normalises / re-encodes to mp3). These are **offline assets like `Alpha.png`, not a runtime
service** — so the deep tier stays **offline-safe**. Licensing is **CC BY-SA 3.0** (one PD clip, `/dʒ/`),
none ND → repitch-safe; attribution lives in **`data/phoneme-credits.json`** + **`sounds/CREDITS.md`**.
Same http-serving requirement as the other sample sets (fetch/decode needs http, not `file://`).

The rejected alternative, for the record: **OS `speechSynthesis`** — local + offline, but the voice is
