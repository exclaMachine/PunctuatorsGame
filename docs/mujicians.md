...
>
> **MVP BUILT 2026-07-26 (`forge-quench.html`, standalone):** the full core loop — immediate-apply **stack-builder** (weld thirds onto a root ingot; **click a tone to select, Temper nudges it**), ops **+M3 / +m3 / Temper ±1 / Undo / 🔥 Quench / ↺ Reset** (keys `a s , . u space r`), **build-to-spec orders** (quality × root × metal, with a live recipe hint + **ghost target silhouette** on the anvil), **strike-test** (Quench sounds the whole chord in the metal's voice → on-spec **rings** / off-spec or untempered-tritone **cracks & shatters**), **economy scoring** (heat-left → chips-style `mult`, `MASTERWORK_MULT 1.45` → bigger damage), **heat budget** (`HAMMER_HEAT 3`/`TEMPER_HEAT 2`, `HEAT_BUDGET 13`; ops disable when unaffordable), **flaw-injection debuff** (phase ≥2, welds a `root+6` tritone you must Temper onto a chord tone — skipped when the target legitimately holds the tritone, i.e. diminished), **cooling** (`COOL_MS 26000` gentle re-heat, not a twitch layer), **3 HP-gated phases** (major/minor → +dim/aug+flaw → dom7 finisher), **Tritony HP / Torch (3 lives)** with hurt/attack/shake juice + ring/crack FX, and **win/lose→retry** end screens with the *Devil's Bargain* capture line. Reuses `classify`/`TRIADS`/`SEVENTHS` + `VOICES`/`playVoice` (5 real instrument metals → voices: brass=reedorgan, bronze=glassbell, steel=grand, aluminum=vibraphone, silver=warmpad). **Deferred (next passes / port):** the "program-then-▶Run arm" batch model (MVP applies ops immediately), the piston layer (Invert / Fifth-frame / 7th-barb), the alloy-ratio metallurgy puzzle, weapon-silhouette art, telegraphed volley phases, and campaign wiring (`run.debuff='dissonant'`, `runThreshold` HP, mentor intro, screen routing, capture → Muse pool).
